...
// aggiungi bevanda all'ordine
func (h *PietanzaHandler) AddBevandaToOrdine(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	idOrdineStr := chi.URLParam(r, "id_ordine")
	idOrdine, err := strconv.Atoi(idOrdineStr)
	if err != nil {
	...
	var requestBody struct {
		IDBevanda int `json:"id_pietanza"`
		Quantita  int `json:"quantita"`
	}
	...
	// Validazione base
	if requestBody.IDBevanda <= 0 || requestBody.Quantita <= 0 {
		http.Error(w, "ID bevanda e quantità devono essere maggiori di zero", http.StatusBadRequest)
		return
	}

	// Aggiungi la bevanda all'ordine
	err = h.repo.AddBevandaToOrdine(ctx, idOrdine, requestBody.IDBevanda, requestBody.Quantita)
	if err != nil {
		switch err {
		case repository.ErrPietanzaNonDisponibile:
			http.Error(w, "La bevanda non è disponibile", http.StatusBadRequest)
		default:
			http.Error(w, "Errore nell'aggiunta della bevanda all'ordine", http.StatusInternalServerError)
			log.Printf("Errore nell'aggiunta della bevanda all'ordine: %v", err)
		}
		return
	}

	w.WriteHeader(http.StatusOK)
	json.NewEncoder(w).Encode(map[string]string{"message": "Bevanda aggiunta all'ordine con successo"})
}
