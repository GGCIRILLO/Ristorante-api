...
func (r *PietanzaRepository) AddBevandaToOrdine(ctx context.Context, idOrdine int, idBevanda int, quantita int) error {
	// Inizia una transazione
	tx, err := r.DB.Begin(ctx)
	if err != nil {
		return err
	...
	// 1. Verifica che la bevanda sia disponibile
	var disponibile bool
	err = tx.QueryRow(ctx, `
		SELECT disponibile
		FROM pietanza
		WHERE id_pietanza = $1
	`, idBevanda).Scan(&disponibile)
	...
	// 2. Aggiunge la bevanda all'ordine
	_, err = tx.Exec(ctx, `
		INSERT INTO dettaglio_ordine_pietanza (id_ordine, id_pietanza, quantita, parte_di_menu, id_menu)
		VALUES ($1, $2, $3, false, 0)
		ON CONFLICT (id_ordine, id_pietanza, parte_di_menu, id_menu)
		DO UPDATE SET quantita = dettaglio_ordine_pietanza.quantita + EXCLUDED.quantita
	`, idOrdine, idBevanda, quantita)

	if err != nil {
		return err
	}

	// 3. Aggiorna il costo totale dell'ordine
	ordineRepo := OrdineRepository{DB: r.DB}
	err = ordineRepo.AggiornaCostoTotale(ctx, tx, idOrdine, nil)
	if err != nil {
		return err
